...
/*
g++ v4l2stream2.cpp -o v4l2stream2 -I/userdata/stream/myusr/include -fpermissive -Wall -Wextra -L/userdata/stream/myusr/lib -Wl,-rpath,/userdata/stream/myusr/lib -lavformat -lavfilter -lavcodec -lavutil -lavdevice -lswscale -lavfilter -lpthread -D_GNU_SOURCE -fpermissive

gcc t.c -o t -I/userdata/stream/myusr/include -fpermissive -Wall -Wextra -L/userdata/stream/myusr/lib -Wl,-rpath,/userdata/stream/myusr/lib -lavformat -lavfilter -lavcodec -lavutil -lavdevice -lswscale -lavfilter -lpthread -D_GNU_SOURCE -fpermissive



./v4l2stream2 /dev/video0 rtsp://192.168.1.86:554/live/stream
./v4l2stream2 /dev/video1 rtsp://192.168.1.86:554/live/stream

v4l2-ctl -d /dev/video1 --stream-mmap --stream-count=100 --stream-to=/dev/null
<<<<<<<<<<<<<<<<<<<< 18.76 fps



linaro@linaro-alip:/userdata/stream/video$ g++ v4l2stream2.cpp -o v4l2stream2 -I/userdata/stream/myusr/include -fpermissive -Wall -Wextra -L/userdata/stream/myusr/lib -Wl,-rpath,/userdata/stream/myusr/lib -lavformat -lavfilter -lavcodec -lavutil -lavdevice -lswscale -lavfilter -lpthread -D_GNU_SOURCE -fpermissive
...
private:
    ...
    int init_v4l2_device() {
        ...
        if (delimiter != std::string::npos) {
            width = std::stoi(config_.video_size.substr(0, delimiter));
            height = std::stoi(config_.video_size.substr(delimiter + 1));
        }
        ...
        //fmt.fmt.pix_mp.pixelformat = V4L2_PIX_FMT_YUV422P; // or your preferred format
        fmt.fmt.pix_mp.pixelformat = V4L2_PIX_FMT_NV12; // or your preferred format
...
int actual_num_planes = actual_fmt.fmt.pix_mp.num_planes;
g_logger.log(LOG_INFO, "Actual num_planes: " + std::to_string(actual_num_planes));
...
/*
        ...
        if (ioctl(v4l2_fd_, VIDIOC_REQBUFS, &req) < 0) {
            g_logger.log(LOG_ERROR, "Failed to request V4L2 buffers11111: " + std::string(strerror(errno)));
            close(v4l2_fd_);
            v4l2_fd_ = -1;
            return -1;
        }
        ...
        for (unsigned int i = 0; i < req.count; ++i) {
            ...
            if (ioctl(v4l2_fd_, VIDIOC_QUERYBUF, &buf) < 0) {
                g_logger.log(LOG_ERROR, "Failed to query V4L2 bufferiiiiiinit-----------: " + std::string(strerror(errno)));
                close(v4l2_fd_);
                v4l2_fd_ = -1;
                return -1;
            }
...
*/
    ...
    int init_framerate_filter() {
        ...
        if (is_rtsp_source()) {
            ...
            pix_fmt = AV_PIX_FMT_NV12; // Assuming NV12 for V4L2
            time_base = (AVRational){1, static_cast<int>(config_.input_fps * 1000)};
        }
        ...
        std::string args = "video_size=" + std::to_string(width) + "x" + 
                          ...
                          std::to_string(sar.den); // dynamic set SAR
    ...
    int init_encoder() {
        ...
        if (is_rtsp_source()) {
            ...
            pix_fmt = static_cast<AVPixelFormat>(in_stream->codecpar->format);
        } else {
            // Parse video size for V4L2
            size_t delimiter = config_.video_size.find('x');
            width = 1280;
...
void capture_loop_v4l2() {
    ...
    if (ioctl(v4l2_fd_, VIDIOC_REQBUFS, &req) < 0) {
        g_logger.log(LOG_ERROR, "Failed to request V4L2 buffers222: " + std::string(strerror(errno)));
        return;
    }

    // Map the buffers
    void* buffer_start[VIDEO_MAX_PLANES] = {NULL};
    size_t buffer_length[VIDEO_MAX_PLANES] = {0};
    ...
    for (unsigned int i = 0; i < req.count; ++i) {
        ...
        if (ioctl(v4l2_fd_, VIDIOC_G_FMT, &fmt2) == 0) {
            for (int j = 0; j < fmt2.fmt.pix_mp.num_planes; j++) {
	       	bytesperline[j] = fmt2.fmt.pix_mp.plane_fmt[j].bytesperline;	   
	        g_logger.log(LOG_INFO, "Query V4L2 buffer: i" +std::to_string(j) + ",bytesperline="+std::to_string(bytesperline[j]));

	        g_logger.log(LOG_INFO, "Query V4L2 buffer: i" +std::to_string(j) + ",bytesperline="+std::to_string(bytesperline[j])+", width="+std::to_string(fmt2.fmt.pix_mp.width)+",height"+std::to_string(fmt2.fmt.pix_mp.height));
            
			      g_logger.log(LOG_INFO, "Query V4L2 buffer: i" +std::to_string(j) + ",fmt="+std::to_string(fmt2.fmt.pix_mp.pixelformat));
        ...
        for (int j = 0; j < VIDEO_MAX_PLANES; j++) {
            ...
            buffer_start[j] = mmap(NULL, planes[j].length,
                                 PROT_READ | PROT_WRITE, MAP_SHARED,
                                 v4l2_fd_, planes[j].m.mem_offset);
            if (buffer_start[j] == MAP_FAILED) {
                g_logger.log(LOG_ERROR, "Failed to mmap V4L2 buffer: " + std::string(strerror(errno)));
                return;
            }
            buffer_length[j] = planes[j].length;
            //bytesperline[j] = planes[j].bytesperline;
            bytesperline[j] = 1280;
        }
    ...
    while (!should_stop_) {
        ...
        if (ret == 0) {
            g_logger.log(LOG_WARNING, "V4L2 select timeout");
            continue;
        }
        ...
        for (int i = 0; i < 1; i++) {
            frame->data[i] = static_cast<uint8_t*>(buffer_start[i]) + planes[i].data_offset;
            frame->linesize[i] = bytesperline[i];
        }
            frame->data[1] = frame->data[0]+(bytesperline[0] * height);
	    frame->linesize[1] = bytesperline[0];// testing
